...
export default function ProjectDetail() {
    ...
    const [createdByName, setCreatedByName] = useState("N/A");
    ...
    const fetchProjectAndTasks = async () => {
        ...
        try {
            ...
            const [projectRes, tasksRes] = await Promise.all([
                getProjectById(id),
                getTasksByProject(id, 0, 1000)
            ]);
            setProject(projectRes.data);
            setTasks(tasksRes.data);

            // Fetch creator name using createdById
            if (projectRes.data?.createdById) {
                try {
                    const userRes = await getUserById(projectRes.data.createdById);
                    setCreatedByName(userRes.data?.name || "N/A");
                } catch {
    ...
    const statusColors = {
        ...
        NOT_STARTED: "bg-amber-200 text-amber-900 dark:bg-amber-500 dark:text-amber-900",
        COMPLETED: "bg-blue-200 text-blue-900 dark:bg-blue-500 dark:text-blue-900",
    };

    if (loading) return <div className="p-6 text-center text-zinc-500">Loading project...</div>;
        ...
        <div className="space-y-5 max-w-6xl mx-auto text-zinc-900 dark:text-white">
            ...
            <div className="grid grid-cols-2 sm:flex flex-wrap gap-6">
                {[
                    ...
                    { label: "Completed", value: tasks.filter((t) => t.status === "COMPLETED").length, color: "text-emerald-700 dark:text-emerald-400" },
                    { label: "In Progress", value: tasks.filter((t) => t.status === "IN_PROGRESS").length, color: "text-amber-700 dark:text-amber-400" },
                    { label: "Created By", value: createdByName, color: "text-cyan-700 dark:text-cyan-400" },
                    { label: "Deadline", value: project.deadline || "N/A", color: "text-amber-700 dark:text-amber-400" },
                ].map((card, idx) => (
                ...
                <div className="mt-6">
                    {activeTab === "tasks" && (
                        <div className="dark:bg-zinc-900/40 rounded max-w-6xl">
                            <ProjectTasks tasks={tasks} onTaskUpdated={fetchProjectAndTasks} />
                        </div>
                    )}
                    {activeTab === "analytics" && (
                        <div className="dark:bg-zinc-900/40 rounded max-w-6xl">
                    ...
                    {activeTab === "settings" && (
                        <div className="dark:bg-zinc-900/40 rounded max-w-6xl">
                            <ProjectSettings project={project} onProjectUpdated={fetchProjectAndTasks} />
                        </div>
                    )}
                </div>
            </div>
