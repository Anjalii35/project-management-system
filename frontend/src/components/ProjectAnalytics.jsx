...
import { CheckCircle, Clock, AlertTriangle, ArrowRightIcon } from "lucide-react";
...
const ProjectAnalytics = ({ project, tasks }) => {
    const { stats, statusData, priorityData } = useMemo(() => {
        ...
        const statusMap = { TO_DO: 0, IN_PROGRESS: 0, COMPLETED: 0 };
        const priorityMap = { LOW: 0, MEDIUM: 0, HIGH: 0 };

        tasks.forEach((t) => {
            if (t.status === "COMPLETED") stats.completed++;
            if (t.status === "IN_PROGRESS") stats.inProgress++;
            if (t.status === "TO_DO") stats.todo++;
            if (t.deadline && new Date(t.deadline) < now && t.status !== "COMPLETED") stats.overdue++;
                        ...
                        <div key={p.name} className="space-y-2">
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2">
                                    <ArrowRightIcon className={`size-3.5 ${PRIORITY_COLORS[p.name]}`} />
