...
const ProjectCalendar = ({ tasks }) => {
    ...
    // Use task.deadline instead of task.due_date
    const getTasksForDate = (date) =>
        tasks.filter((task) => task.deadline && isSameDay(new Date(task.deadline), date));

    const upcomingTasks = tasks
        .filter((t) => t.deadline && !isBefore(new Date(t.deadline), today) && t.status !== "COMPLETED")
        .sort((a, b) => new Date(a.deadline) - new Date(b.deadline))
        .slice(0, 5);

    const overdueTasks = tasks.filter(
        (t) => t.deadline && isBefore(new Date(t.deadline), today) && t.status !== "COMPLETED"
    );
                    ...
                    <div className="grid grid-cols-7 gap-2">
                        {daysInMonth.map((day) => {
                            ...
                            const hasOverdue = dayTasks.some(
                                (t) => t.status !== "COMPLETED" && isBefore(new Date(t.deadline), today)
                            );
                            return (
                                <button
                                    key={day.toISOString()}
                    ...
                    <div className="not-dark:bg-white mt-6 dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4">
                        ...
                        <div className="space-y-3">
                            {getTasksForDate(selectedDate).map((task) => (
                                <div key={task.id} className={`bg-zinc-50 dark:bg-zinc-800/40 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition p-4 rounded border-l-4 ${priorityBorders[task.priority]}`}>
                                    <div className="flex justify-between mb-2">
                                        <h4 className="text-zinc-900 dark:text-white font-medium">{task.title}</h4>
                                        <span className="px-2 py-0.5 rounded text-xs bg-zinc-200 dark:bg-zinc-700 text-zinc-800 dark:text-zinc-200">
                                            {task.priority}
                                        </span>
                                    </div>
